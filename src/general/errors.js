...
class AllRoundsFinished extends Error {
    constructor() {
        ...
        loggers.silly_logger(AllRoundsFinished, arguments, 'general')
        this.code = 549
...
class NoRollBack extends Error {
    constructor() {
        super('NoRollBack')
        loggers.silly_logger(NoRollBack, arguments, 'general')
        this.code = 550
...
class DoesNotExist extends Error {
    constructor(identity) {
        super('DoesNotExist')
        loggers.silly_logger(DoesNotExist, arguments, 'general')
...
class AlreadyExists extends Error {
    constructor(identity) {
        super('AlreadyExists')
        loggers.silly_logger(AlreadyExists, arguments, 'general')
        this.identity = identity
        this.code = 552
        this.text = 'The target with identity '+this.identity.toString()+'does not exist'
...
class ResultNotSent extends Error {
    constructor(id, role) {
        super('ResultNotSent')
        loggers.silly_logger(ResultNotSent, arguments, 'general')
        this.id = id
        this.role = role
        this.code = 553
...
class WinPointsDifferent extends Error {
    constructor(id, wins) {
        ...
        loggers.silly_logger(WinPointsDifferent, arguments, 'general')
        this.id = id
        this.wins = wins
        this.code = 554
...
class NeedMore extends Error {
    constructor(role, atleast) {
        ...
        loggers.silly_logger(NeedMore, arguments, 'general')
        this.role = role
...
class DebaterNotRegistered extends Error {
    constructor(id, r) {
        ...
        loggers.silly_logger(DebaterNotRegistered, arguments, 'general')
        this.id = id
        this.r = r
        this.code = 556
...
class EntityNotDefined extends Error {
    constructor(id, role) {
        ...
        loggers.silly_logger(EntityNotDefined, arguments, 'general')
        this.id = id
        this.role = role
        this.code = 557
