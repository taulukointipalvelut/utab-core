...
class Main {
    constructor (name) {
        var con = new controllers.CON(name)
        ...
        this.allocations = {//op.allocations
            get: function({
                    ...
                    filters_adj2: filter_functions_adj2_strs=['by_conflict', 'by_institution', 'by_past']
                }, allocation) {
                try {
                    var all_filter_functions = op.allocations.teams.functions.read()
                    var all_filter_functions_adj = op.allocations.adjudicators.functions.read()
